...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.array(v_list).flatten() # or reshape...
        Q = scipy.linalg.block_diag(*Q_list)
        H = np.vstack(H_list) #reshape(-1, H_list[0].shape[1])
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        #       if the minimum distance satisfies the gating criteria, add corresponding entries to v_list, Q_list, H_list
         ...
         # I - observatinos, J is the number of features
        I = len(z_raw)
        J = len(hs)
        ...
        for i in range(I):
            ...
            for j in range(J):        
                ...
                v_ij = np.array([angle_diff(z_raw[i,0], hs[j,0]), z_raw[i,1] - hs[j,1]])
                
                S = ((Hs[j] @ self.Sigma) @ Hs[j].T) + Q_raw[i]
                d_ij = np.dot((v_ij.T @ np.linalg.inv(S)), v_ij)

                if d_ij < dmin:
                    dmin = d_ij # reset value of d
                    q, h, v = Q_raw[i], Hs[j], v_ij
                   

            if dmin < pow(self.g, 2):
                v_list.append(v)
                Q_list.append(q)
                H_list.append(h)
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        xvec = self.x[0:3]
        g_0, Gx0, Gu0 = tb.compute_dynamics(xvec, u, dt, compute_jacobians=True)
        g[:3] = g_0
        Gx[:3,:3] = Gx0
        Gu[:3,:2] = Gu0
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            currentLine = np.array([alpha, r])

            # Derive the Angle from the base frame to the given camera.
            x_base, y_base, theta = self.x[0],self.x[1], self.x[2] 
            ycam_base, xcam_base = self.tf_base_to_camera[1], self.tf_base_to_camera[0]
            theta_btc = np.arctan2(ycam_base, xcam_base)
            cam_base_transformation = np.linalg.norm(np.array([xcam_base, ycam_base]))

            # HINT: The first 3 columns of Hx, populated using the same approach as in EkfLocalization.compute_predicted_measurements().
            h, Hx_slam = tb.transform_line_to_scanner_frame(currentLine, self.x, self.tf_base_to_camera) # change to self.x
            x_dral = (x_base +  cam_base_transformation*np.cos(theta_btc + theta))*np.sin(alpha)
            y_dral = -1 * (y_base +  cam_base_transformation*np.sin(stheta_btc + theta))*np.cos(alpha)

            

            # Derive Derivatives of x and y in refernce to alpha base x_0 + d*r*t
            if j >= 2:

                # Below Doesn't works
                Hx[:,idx_j:idx_j+2] = np.eye(2)  # FIX ME!
                Hx[:,idx_j:idx_j+2] = np.array([[1,0], [0, round(x_dral + y_dral)]]).astype('int32')

            Hx[:,0:3] = Hx_slam # Original Hx is still the Hx, for columns 0:3....
