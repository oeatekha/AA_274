...
# Exploration
# you detect an animal -> clicking clicking clicking, publish to **animal poses** 
# animal poses publishes to goal commander and stores the nav goals within the map: animal name + pose
# navigator.py always run animal postion detector publisher. When the state is set to rescue, 
# Rescue
# we publish the first in list [animal: x1,y1, z1, theta1; x2, y2, z2, theta2] to goal_commander

# Exploration
# We need to get detector_mobilenet running and detecing objects, map + animals + teleop detect animals
# goal_commander subscriber for detector_mobilenet (CNN) and wait for a new "animal to be seen
# create a list of animals, if you see the same animal update, pose
# detector mobilenet publish [list of animals], if new published list is != list that we have been seeing 
# update the list of poses to navigate towards, goal_commander,listens both to detector and gmapping and appends a 2d
# pose.


# if using gmapping, you will have a map frame. otherwise it will be odom frame
# when we run goal commander we need to give it an input of where to go next, or a name and pose it will grab 
# from the detected animals

# easiest way we can give an input to a ros system 
# have a publisher maybe listens for a keyboard input publish that topic to goal commander current 
# destination and go to said object
